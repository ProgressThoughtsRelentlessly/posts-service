...
@Service
//@RequestScope
public class AuthorStrategy implements MiniPostStrategy {
...
//	public AuthorStrategy setAuthor(String author) {
//		this.author = author;
//		return this;
//	}
//	
...
// todo: refer: https://stackoverflow.com/questions/59588995/mongodb-iterate-over-keys-of-document-sum-values

	@Override
	public List<MiniPost> getMiniPosts(int page, MiniPostClientInput input) {
		// TODO fetch postIds wrt authors from userPostMetadata and fetch miniPost based on those postIds
		String author = input.getInputString();
		List<UserProfile> userProfileList = userProfileRepository.findByFirstnameOrLastnameOrEmail(author , author, author);
		List<UserPostMetadata> userPostMetadataList =
						userProfileList
						.stream()
						.flatMap(user -> user.getUserPostMetadata().stream())
						.collect(Collectors.toList());
		List<ObjectId> postIds = userPostMetadataList.stream().map(metadata -> new ObjectId(metadata.getPostId())).collect(Collectors.toList());
